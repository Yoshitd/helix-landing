...
export function Process() {
  ...
  return (
    <section id="process" ref={ref} className="relative h-[320vh] bg-[#0b1413]">
      <div className="sticky top-0 flex h-screen flex-col justify-center overflow-hidden">
        <div className="mx-auto w-full max-w-6xl px-6">
          ...
          {/* progress line */}
          <div className="mt-8 h-px w-full max-w-xl overflow-hidden bg-white/10">
        ...
        <motion.div style={{ x }} className="mt-12 flex gap-6 pl-[max(1.5rem,calc((100vw-72rem)/2+1.5rem))]">
          {steps.map((s) => (
            <article
              key={s.no}
              className="relative flex h-[22rem] w-[80vw] shrink-0 flex-col justify-between rounded-3xl border border-white/10 bg-white/[0.03] p-9 backdrop-blur-sm sm:w-[26rem]"
            >
